"use strict";
var dataTablePch;
var orgSchemaId = "59f80333-9def-46c5-9670-99a0f2947934";

var TabelPch = (function () {
    var initDataTable = function () {
        dataTablePch = $("#table-1").DataTable({
            processing: true,
            serverSide: true,
            dom: "tipr",
            order: [],
...
$(document).ready(function() {
    // Initialize Leaflet map for map-1
    
    map1 = L.map('map-1').setView([51.483, -0.090], 13);

    var linePrimer = L.polyline([
        [51.505, -0.01],
        [51.505, -0.160],
    ], { color: 'green', weight: 5}).addTo(map1);
    ...
    footer += '<div class="row"><div class="yellow-circle"></div>&nbsp: Manual</div><br>';
    footer += '<div class="row"><div class="orange-circle"></div>&nbsp: Manual & Telemetri (KOICA)</div><br>';
    footer += '<div class="row"><div class="green-circle"></div>&nbsp: Telemetri (KOICA)</div><br>';
    footer += '<div class="row"><div class="purple-circle"></div>&nbsp: Manual & Telemetri (PUPR& BMKG)</div><br>';
    ...
    */
    ...
    var lineBlackCitarum = L.polyline([
        [51.505, -0.115],
        [51.430, -0.115]
    ], { color: 'black', weight: 5}).addTo(map1);
    ...
    */
    ...
    var lineGreyTarumBaratTimur = L.polyline([
        [51.490, -0.055],
        [51.490, -0.160]
    ], { color: 'grey', weight: 5}).addTo(map1);
    ...
    */
    ...
    var bdBekasi1 = L.polyline([
        ...
        [51.494, -0.154],
        [51.495, -0.152]
    ], {color: 'green', weight: 2}).addTo(map1);
    ...
    var lineBox1 = L.polyline([
        [51.433, -0.110],
        [51.433, -0.115],
    ], {color: 'blue', weight: 1}).addTo(map1);

    var lineBox4 = L.polyline([
        [51.440, -0.110],
        [51.440, -0.115],
    ], {color: 'blue', weight: 1}).addTo(map1);

    var lineBox5 = L.polyline([
        [51.436, -0.083],
        [51.439, -0.083],
    ], {color: 'blue', weight: 1}).addTo(map1);

    var lineBox9 = L.polyline([
        [51.448, -0.106],
        [51.449, -0.105],
    ], {color: 'blue', weight: 1}).addTo(map1);

    var lineBox15 = L.polyline([
        [51.454, -0.118],
        [51.456, -0.115],
    ], {color: 'blue', weight: 1}).addTo(map1);

    var lineBox19 = L.polyline([
        [51.458, -0.128],
        [51.460, -0.128],
    ], {color: 'blue', weight: 1}).addTo(map1);

    var lineBox21 = L.polyline([
        [51.458, -0.133],
        [51.455, -0.133],
    ], {color: 'blue', weight: 1}).addTo(map1);

    var lineBox24 = L.polyline([
        [51.458, -0.103],
        [51.456, -0.104],
    ], {color: 'blue', weight: 1}).addTo(map1);

    var lineBox30 = L.polyline([
        [51.470, -0.055],
        [51.469, -0.060],
    ], {color: 'blue', weight: 1}).addTo(map1);

    var lineBox31 = L.polyline([
        [51.473, -0.073],
        [51.473, -0.069],
    ], {color: 'blue', weight: 1}).addTo(map1);

    var lineBox32 = L.polyline([
        [51.478, -0.073],
        [51.475, -0.078],
    ], {color: 'blue', weight: 1}).addTo(map1);

    var lineBox34 = L.polyline([
        [51.468, -0.137],
        [51.467, -0.133],
    ], {color: 'blue', weight: 1}).addTo(map1);

    var lineBox35 = L.polyline([
        [51.440, -0.128],
        [51.441, -0.132],
    ], {color: 'blue', weight: 1}).addTo(map1);

    var lineBox36 = L.polyline([
        [51.481, -0.122],
        [51.478, -0.125],
    ], {color: 'blue', weight: 1}).addTo(map1);

    var lineBox37 = L.polyline([
        [51.475, -0.115],
        [51.475, -0.120],
    ], {color: 'blue', weight: 1}).addTo(map1);

    var lineBox38 = L.polyline([
        [51.504, -0.103],
        [51.509, -0.103],
    ], {color: 'blue', weight: 1}).addTo(map1);
    ...
    var rectanglePch = getDataRectangleCurahHujan();

    rectanglePch.then(resRectangle => {
        resRectangle.forEach(rec => {
            ...
            const rectangle = L.rectangle(rectangleCoords, rectangleOptions).addTo(map1);

            const textElement = document.createElement('div');

            getDataPosByStationIdPch(rec.stationId).then(readRec => {
                if(readRec.result.readingAt != null) {
                    textElement.innerHTML = rec.no + '.CH: <b>' + readRec.result.rainfall.toFixed(2) + ' mm</b>';
                }
            }).catch(error => {
            ...
            map1.getPane('overlayPane').appendChild(textElement);

            // Function to update the text position based on rectangle bounds
            function updateTextPosition() {
                const bounds = rectangle.getBounds();
                const topLeft = map1.latLngToLayerPoint(bounds.getNorthWest());
                const bottomRight = map1.latLngToLayerPoint(bounds.getSouthEast());
            ...
            map1.on('zoomend', updateTextPosition);
            map1.on('moveend', updateTextPosition);
            //end rectangle data
        });
    }).catch(error => {
        console.error(error);
    ...
    var circlePch = getDataSebaranPch();

    circlePch.then(results => {
        results.forEach(r => {
            ...
            getDataPosByStationIdPch(r.stationId)
                .then(reading => {
                    // Bind popup when data is available
                    textMarker.bindPopup(createDetailPosPch(reading));
                })
                .catch(error => {
                    console.error('Error fetching data:', error);
                });
...
async function getDataSebaranPch() {
    try {
        var orgCode = "59f80333-9def-46c5-9670-99a0f2947934";
...
async function getDataRectangleCurahHujan() {
    try {
        var orgCode = "59f80333-9def-46c5-9670-99a0f2947934";
...
async function getDataPosByStationIdPch(stationId) {
    try {
        if(stationId != null) {
            const response = await fetch(`/StationSchema/GetDataPosPchByStationId?idStation=${stationId}`);
            if (response.ok) {
...
function createDetailPosPch(reading) {
    ...
    if(reading.result.readingAt != null) {
        ...
        panelContent += `<td class="py-1 px-2">:</td>`;
        panelContent += `<td class="py-1 px-0 fw-semibold">${reading.result.rainfall.toFixed(2)} <sup>mm</sup></td>`;
        panelContent += '</tr>';

        panelContent += '<tr>';
        panelContent += `<td class="py-1 px-0 nowrap">Curah Hujan Maks.</td>`;
        panelContent += `<td class="py-1 px-2">:</td>`;
        panelContent += `<td class="py-1 px-0 fw-semibold">${reading.result.rainfallMax.toFixed(2)} <sup>mm</sup></td>`;
        panelContent += '</tr>';
    }
